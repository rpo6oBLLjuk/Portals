...
namespace CustomInspector.Extensions
{
    public static class DrawProperties
    {
        ...
        public static void PropertyFieldWithoutLabel(Rect position, SerializedProperty property, bool includeChildren = true)
        {
            ...
            {
                GUIContent label = PropertyValues.ValidateLabel(null, property);
                label.text = "";
                DrawProperties.PropertyField(position: position,
                                                    label: label,
                                                    property: property,
            ...
            else
            {
                ...
                else
                {
                    ...
                    EditorGUI.LabelField(position: position, label: label);
                }
            }
        }
        ...
        public static void DrawLabelSettings(Rect position, SerializedProperty property, GUIContent label, InternalLabelStyle style)
        {
            switch (style)
            {
                ...
                case InternalLabelStyle.NoSpacing:
                    ...
                    {
                        label = PropertyValues.ValidateLabel(label, property); //we have to repair before checking width
                        float labelWidth = GUI.skin.label.CalcSize(label).x + 5; //some additional distance
                        using (new LabelWidthScope(labelWidth))
                        {
                            EditorGUI.PropertyField(position, property, label); //we dont need DrawProperties.PropertyField since we already repaired the label
                        }
                    }
                    break;
        ...
        public const float messageBoxStartSpacing = 5;
        ...
        {
            return DrawProperties.messageBoxHeight + DrawProperties.messageBoxStartSpacing
                   + GetPropertyHeight(label, property, includeChildren) + messageBoxEndSpacing;
        }
        ...
        public static class ListDrawer
        {
            ...
            public static class ReadOnly
            {
                ...
                public static void DrawList<T>(Rect position, SerializedProperty owner, IEnumerable<T> list, bool withLabels = true)
                {
                    ...
                    owner.isExpanded = EditorGUILayout.Foldout(owner.isExpanded, PropertyValues.ValidateLabel(null, owner));
                    if (owner.isExpanded)
                    {
                        using (new EditorGUI.IndentLevelScope(1))
                        {
