...
namespace GameDevWithLukas
{
    public class RigidBodyController : MonoBehaviour
    {
        ...
        //offset: xyz;
        //velocity: xy;
        //force: rb.velocity + offset + velocity;

        //endForce: force / (1+ (y^2)*5)
        protected virtual void HorizontalMove(float _deltaTime)
        {
            ...
            float3 offset = isInputIdle ? float3.zero : math.normalize(_moveDirection) * deltaSpeed; //направление * скрость * Time.deltaTime

            float3 velocity = offset / _deltaTime; //направление * скрость
